...
const Projects = () => {
  ...
  const { ref: gridRef, hasBeenInView: gridInView } = useIntersectionObserver();
  // ------------------------------------------
            ...
            {filteredProjects.map((project, index) => (
              <Card
                ...
                className={`overflow-hidden border-2 border-transparent 
                            ...
                            hover:transition-all hover:duration-200 hover:ease-in-out ${ // Apply faster duration to ALL hover transitions
                  gridInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20"
                }`}
                style={{ transitionDelay: `${index * 100}ms` }} // Staggered scroll-in
              >
